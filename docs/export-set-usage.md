...
| --- | --- | --- |
| `<set-slug>` | Set id from the root `manifest.yaml` (e.g. `welpen-grundkurs-from-de`) or the folder name of the set path (e.g. `welpen-grundkurs` for `sets/de/welpen-grundkurs`) | required |
...
# Large set: split into parts of at most 3 lessons each for an AI with a
# limited context window (welpen-grundkurs has 4 lessons, so this gives a
# 3+1 split)
