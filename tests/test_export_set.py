...
* the manifest set id (``welpen-grundkurs-from-de``) resolves the same set
  as the path basename slug (``welpen-grundkurs``).
"""
...
KNOWN_SLUG = "welpen-grundkurs"
KNOWN_MANIFEST_ID = "welpen-grundkurs-from-de"
KNOWN_SET_DIR = REPO_ROOT / "sets" / "de" / "welpen-grundkurs"
...
def test_unknown_slug_fails_and_lists_available_sets(tmp_path: Path, capsys) -> None:
    ...
    assert KNOWN_MANIFEST_ID in captured_stderr
